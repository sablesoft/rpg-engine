...
3. Define the scenario's opening-scene policy as one of:
   - one prepared opening scene
   - a small prepared choice set of opening scenes
   - dynamic opening-scene generation guidance
4. Ensure the scenario title and any newly introduced entity names are distinct from other established entity names inside that same world context unless the same entity is intentionally being revised.
5. Create or update a world-owned scenario structure following `products/rpg-engine/assets/workspace_templates/scenario_structure.md` and using:
...
6. Ensure the scenario depends on world canon instead of redefining it.
7. Keep enough structure for later adventure setup and play.
8. When the opening scene is prepared rather than dynamic, ensure the scenario owns the reusable scene definition under its `scenes/` folder.
